...
def eval_val_sliding_ttt(
    ...
    """Legal score-first TTT: score each chunk before any update that uses it."""
    seq_len = h.eval_seq_len
    total_tokens = val_data.val_tokens.numel() - 1
    ...
    window_starts = [
        ...
        if min(ws + seq_len, total_tokens) - ws >= h.eval_stride or ws == 0
    ]
    ...
    for ws in window_starts:
        ...
        wlen = end - ws
        scored_offset = 0 if ws == 0 else max(wlen - h.eval_stride, 0)
    ...
    log(
        ...
        f"total_windows={len(window_starts)} stride={h.eval_stride} "
    ...
    for chunk_index in range(num_chunks):
        ...
        my_window_start = (len(windows) * h.rank) // h.world_size
        my_window_end = (len(windows) * (h.rank + 1)) // h.world_size
        ...
        with torch.inference_mode():
            for batch_start in range(0, len(my_windows), batch_seqs):
                ...
                for i, ws in enumerate(batch_ws):
                    ...
                    wlen = end - ws
                    wlens.append(wlen)
                    chunk = val_data.val_tokens[ws:end + 1].to(dtype=torch.int64, device=device)
                    x_batch[i, :wlen] = chunk[:-1]
                    y_batch[i, :wlen] = chunk[1:]
                ...
                for i, ws in enumerate(batch_ws):
                    wlen = wlens[i]
                    score_start = 0 if ws == 0 else max(wlen - h.eval_stride, 0)
        ...
        if not is_last_chunk and h.ttt_epochs > 0:
            base_model.train()
            chunk_seqs = (chunk_end - chunk_start) // h.train_seq_len
            if chunk_seqs > 0:
                cos_lr = h.ttt_lr * 0.5 * (
                    1.0 + math.cos(math.pi * chunk_index / max(num_chunks - 1, 1))
                )
                for group in optimizer.param_groups:
                    group["lr"] = cos_lr

                my_seq_start = (chunk_seqs * h.rank) // h.world_size
                my_seq_end = (chunk_seqs * (h.rank + 1)) // h.world_size
                my_chunk_seqs = my_seq_end - my_seq_start
                for _ in range(h.ttt_epochs):
                    for batch_start in range(0, my_chunk_seqs, h.ttt_batch_seqs):
                        batch_end = min(batch_start + h.ttt_batch_seqs, my_chunk_seqs)
                        actual_batch_start = my_seq_start + batch_start
                        start_tok = chunk_start + actual_batch_start * h.train_seq_len
                        end_tok = chunk_start + (my_seq_start + batch_end) * h.train_seq_len + 1
                        if end_tok > val_data.val_tokens.numel():
                            continue
                        local = val_data.val_tokens[start_tok:end_tok].to(
                            device=device, dtype=torch.int64
                        )
                        x = local[:-1].reshape(-1, h.train_seq_len)
                        y = local[1:].reshape(-1, h.train_seq_len)
        ...
        if h.is_main_process and (chunk_index % 10 == 0 or chunk_index == num_chunks - 1):
            ...
            log(f"  ttt_chunk [{chunk_index + 1}/{num_chunks}] bpb={running_bpb:.6f} time={elapsed:.1f}s")
...
def train_and_eval(h: Hyperparameters, device: torch.device) -> None:
    ...
    if h.ttt_enabled:
        ...
        ttt_loss, ttt_bpb = eval_val_sliding_ttt(h, device, val_data, eval_model)
        torch.cuda.synchronize()
        log(
            f"legal_ttt val_loss:{ttt_loss:.8f} val_bpb:{ttt_bpb:.8f} "
