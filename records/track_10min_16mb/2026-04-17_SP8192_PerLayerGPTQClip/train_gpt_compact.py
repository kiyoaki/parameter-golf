...
def gptq_mixed_quantize(state_dict,hessians,h):
	...
	for(name,tensor)in state_dict.items():
		...
		cs=h.embed_clip_sigmas if'tok_emb'in name else h.attn_clip_sigmas if'.attn.'in name else h.mlp_clip_sigmas if'.mlp.'in name else h.matrix_clip_sigmas;bits=h.embed_bits if'tok_emb'in name else h.matrix_bits;q,s=gptq_quantize_weight(t,hessians[name],clip_sigmas=cs,clip_range=2**(bits-1)-1);result[name+'.q']=q;result[name+'.scale']=s;meta[name]=f"gptq (int{bits})"
