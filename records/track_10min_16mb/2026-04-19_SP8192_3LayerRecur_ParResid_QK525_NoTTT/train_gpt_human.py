...
class Block(nn.Module):
	def __init__(self,dim,num_heads,num_kv_heads,mlp_mult,rope_base,qk_gain_init,train_seq_len,layer_idx=0,ln_scale=False):super().__init__();self.attn_norm=RMSNorm();self.mlp_norm=RMSNorm();self.attn=CausalSelfAttention(dim,num_heads,num_kv_heads,rope_base,qk_gain_init,train_seq_len);self.mlp=MLP(dim,mlp_mult);self.attn_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.mlp_scale=nn.Parameter(torch.ones(dim,dtype=torch.float32));self.resid_mix=nn.Parameter(torch.stack((torch.ones(dim),torch.zeros(dim))).float());self.parallel=False;self.ln_scale_factor=1./math.sqrt(layer_idx+1)if ln_scale else 1.
	def forward(self,x,x0):mix=self.resid_mix.to(dtype=x.dtype);x_in=mix[0][None,None,:]*x+mix[1][None,None,:]*x0;attn_out=self.attn(self.attn_norm(x_in)*self.ln_scale_factor);x_out=x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out if not self.parallel else x_in+self.attn_scale.to(dtype=x_in.dtype)[None,None,:]*attn_out+self.mlp_scale.to(dtype=x_in.dtype)[None,None,:]*self.mlp(self.mlp_norm(x_in)*self.ln_scale_factor);x_out=x_out+self.mlp_scale.to(dtype=x_out.dtype)[None,None,:]*self.mlp(self.mlp_norm(x_out)*self.ln_scale_factor)if not self.parallel else x_out;return x_out
class GPT(nn.Module):
	def __init__(self,h):
		super().__init__()
...
def apply_qni_(targets,scale):
	...
	return noises
@torch.no_grad()
def revert_qni_(noises):
	for(param,noise)in noises:param.sub_(noise)
@torch.compile
...
def train_model(h,device,val_data):
	...
	if h.parallel_residual_start>=0:log(f"parallel_residuals: ON (layers {h.parallel_residual_start}-{h.num_layers-1})")
	optimizers=Optimizers(h,base_model);train_loader=ShuffledSequenceLoader(h,device);qni_targets=build_qni_targets(base_model,h);max_wallclock_ms=1e3*h.max_wallclock_seconds if h.max_wallclock_seconds>0 else None
	...
	def training_frac(step,elapsed_ms):
		if h.training_frac_mode=='step' or max_wallclock_ms is None:return step/max(h.iterations,1)
		if h.training_frac_mode!='wallclock':raise ValueError(f"Unknown TRAINING_FRAC_MODE: {h.training_frac_mode!r}")
