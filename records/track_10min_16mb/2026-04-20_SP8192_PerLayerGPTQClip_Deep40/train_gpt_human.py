...
class Hyperparameters():
    ...
    attn_clip_sigmas = float(os.environ.get('ATTN_CLIP_SIGMAS', os.environ.get('MATRIX_CLIP_SIGMAS', 12.85)))
    mlp_clip_sigmas = float(os.environ.get('MLP_CLIP_SIGMAS', os.environ.get('MATRIX_CLIP_SIGMAS', 12.85)))
    deep_mlp_clip_sigmas = float(os.environ.get('DEEP_MLP_CLIP_SIGMAS', os.environ.get('MLP_CLIP_SIGMAS', os.environ.get('MATRIX_CLIP_SIGMAS', 12.85))))
    deep_mlp_fraction = float(os.environ.get('DEEP_MLP_FRACTION', 0.0))
...
def gptq_mixed_quantize(
    ...
    if h.deep_mlp_fraction > 0 and h.deep_mlp_clip_sigmas != h.mlp_clip_sigmas:
        deep_mlp_start = min(h.num_layers, max(0, math.ceil(h.num_layers * (1.0 - h.deep_mlp_fraction))))
        log(
            "GPTQ MLP clip split: "
            f"shallow_mid={h.mlp_clip_sigmas:.4f} "
            f"deep={h.deep_mlp_clip_sigmas:.4f} "
            f"deep_start_layer={deep_mlp_start}"
        )
    ...
    for name, tensor in state_dict.items():
        ...
        if "tok_emb" in name:
            ...
            layer_idx = int(match.group(1)) if match is not None else -1
            if deep_mlp_start is not None and layer_idx >= deep_mlp_start:
                cs = h.deep_mlp_clip_sigmas
            else:
